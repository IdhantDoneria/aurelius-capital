...
Production runbook for the Aurelius Capital quant research OS. Assumes a single
...
git clone <repo> /opt/aurelius && cd /opt/aurelius
```
...
- Create `/opt/aurelius/.env.production` (mode `600`, owned by the deploy user)
...
| Metrics | `GET /metrics` (Prometheus text) | `aurelius_up`, uptime, build_info |
...
Point Prometheus at `/metrics`; alert on `aurelius_up == 0` or a jump in
`aurelius_uptime_seconds` resetting (crash-loop). Container restarts are handled
...
0 * * * * cd /opt/aurelius && set -a && . ./.env.production && \
          BACKUP_DIR=/var/backups/aurelius ./scripts/backup.sh >> /var/log/aurelius-backup.log 2>&1
```

Restore: `./scripts/restore.sh /var/backups/aurelius/<file>.dump` (destructive —
...
`aurelius.assistant` reads papers/reports/code and writes hypotheses + reports.
