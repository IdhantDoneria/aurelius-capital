...
def build_visualizations(pm, *, summaries: dict, window: int = 63) -> dict:
    ...
    charts = {
        ...
        "rolling_volatility": _rolling(
            r, window, lambda x: float(np.std(x, ddof=1) * np.sqrt(252))
        ),
        "turnover": {
            "annual_turnover": pm.annual_turnover,
    ...
    if cap.get("estimated_capacity_aum"):
        charts["capacity_curve"] = {
            "aum": cap["aum"],
            "capacity_aum": cap["estimated_capacity_aum"],
