...
class ForwardCampaign:
    ...
    def resume(cls, spec, logic, data_dir: str | Path) -> ForwardCampaign:
        ...
        if not manifest_path.exists():
            raise FileNotFoundError(f"No campaign manifest at {manifest_path}. Use init() first.")
    ...
    def run(self, as_of: date, *, provider_records: list | None = None) -> CycleResult:
        ...
        if existing is not None:
            self._health["already_sealed_skips"] = self._health.get("already_sealed_skips", 0) + 1
        ...
        if not self._scheduler.is_due(spec, rs, as_of):
            ...
            rec.skip_reason = f"not_due — monthly scheduler; last_eval_date={rs.last_eval_date}"
        ...
        if (
            self._config.min_universe_coverage > 0.0
            and coverage < self._config.min_universe_coverage
        ):
            rec.status = CycleStatus.FAILED
            rec.error_message = (
        ...
        # Count diagnostics
        sum(1 for _ in getattr(build_result, "raw_payloads_count", (None,)))
        pit_v = 0
        stale_v = 0
        ...
        rec.gross_return = (
            (rec.ending_nav - rec.starting_nav) / rec.starting_nav if rec.starting_nav > 0 else 0.0
        )
        rec.net_return = rec.gross_return  # net = gross for paper (fees in slippage)

        # positions snapshot
        rec.positions = {sid: h.shares for sid, h in book.state.holdings.items() if h.shares != 0}
        ...
        if sr:
            ...
            rec.risk_decision = (
                str(getattr(sr, "evaluation", None) and getattr(sr.evaluation, "risk_decision", ""))
                or ""
            )
            if sr.evaluation:
                ev = sr.evaluation
    ...
    def status(self) -> dict:
        ...
        return {
            ...
            "strategy_fingerprint": self._spec.configuration_fingerprint
            or self._spec.fingerprint(),
            "mode": "PAPER_FORWARD",
            ...
            "next_expected_cycle": (next_expected.isoformat() if next_expected else None),
            "checkpoint_exists": self._checkpoint_path.exists(),
            "data_limitation": (
                "Yahoo Finance (yfinance) — free/public, NOT institutional/exchange-grade."
            ),
            "governance": (
                "EXPERIMENTAL PAPER TRADING — NOT PRODUCTION APPROVED. NO REAL CAPITAL DEPLOYED."
            ),
    ...
    def _make_partial_record(
        self, cycle_id: str, as_of: date, start_time: datetime
    ) -> ForwardCycleRecord:
