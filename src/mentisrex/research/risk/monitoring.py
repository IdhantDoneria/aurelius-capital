...
from mentisrex.research.risk.models import RiskAlert, RiskDecision, RiskEvent, RiskSnapshot
...
def monitor(reports, *, thresholds: MonitorThresholds | None = None) -> dict:
    ...
    for i, r in enumerate(reports):
        ...
        if r.violations:
            events.append(
                RiskEvent(
                    r.as_of, "limit_breach", {"violations": [v.message for v in r.violations]}
                )
            )
        ...
        if r.drawdown and r.drawdown.halt_triggered:
            events.append(
                RiskEvent(r.as_of, "drawdown_breach", {"current": r.drawdown.current_drawdown})
            )
            alerts.append(
                RiskAlert(
                    r.as_of,
                    "drawdown_breach",
                    f"drawdown halt {r.drawdown.current_drawdown:.3f}",
                    "critical",
                )
            )
        if prev and abs(r.exposure.gross - prev.exposure.gross) > t.exposure_drift:
            events.append(
                RiskEvent(
                    r.as_of, "exposure_drift", {"from": prev.exposure.gross, "to": r.exposure.gross}
                )
            )
            alerts.append(
                RiskAlert(
                    r.as_of,
                    "exposure_drift",
                    f"gross {prev.exposure.gross:.2f}->{r.exposure.gross:.2f}",
                    "warning",
                )
            )
        if trailing_vol and r.volatility > t.vol_spike * (sum(trailing_vol) / len(trailing_vol)):
            ...
            alerts.append(
                RiskAlert(r.as_of, "vol_spike", f"vol spike {r.volatility:.3f}", "warning")
            )
        if (
            prev
            and r.concentration.herfindahl - prev.concentration.herfindahl
            > t.concentration_increase
        ):
            alerts.append(
                RiskAlert(
                    r.as_of,
                    "concentration_increase",
                    f"HHI {prev.concentration.herfindahl:.3f}->{r.concentration.herfindahl:.3f}",
                    "warning",
                )
            )
        if r.liquidity and r.liquidity.liquidity_signal == t.liquidity_signal:
            alerts.append(
                RiskAlert(
                    ...
                    f"liquidity {r.liquidity.liquidity_signal}",
                    "warning",
                )
            )
        if r.volatility > 0:
