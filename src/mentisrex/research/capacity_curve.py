...
def capacity_curve(signals, forward_returns, adv, *, cost_model, q=5,
                   aum_levels=None, periods_per_year=12) -> dict:
    ...
    for aum in aum_levels:
        net = _leg_returns_and_impact(signals, forward_returns, adv, q=q, aum=aum,
                                      cost_model=cost_model)
        sh = sharpe(net, periods=periods_per_year) if net.size >= 2 else float("nan")
        curve.append({"aum": float(aum), "net_sharpe": float(sh),
    ...
    for pt in curve:
        if base == base and base > 0 and pt["net_sharpe"] < base / 2.0:
            half_cap = pt["aum"]
            break
    return {"curve": curve, "base_sharpe": float(base),
            "half_sharpe_capacity_usd": half_cap}
