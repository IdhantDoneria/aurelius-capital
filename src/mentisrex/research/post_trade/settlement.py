...
class SettlementEngine:
    ...
    def instruct(self, *, instruction_id: str, trade_id: str, security_id: str,
                 quantity: float, cash_amount: float, trade_date: date) -> SettlementInstruction:
        sd = settlement_date(trade_date, self.config.horizon(security_id), self.config.holidays)
        inst = SettlementInstruction(
            instruction_id=instruction_id, trade_id=trade_id, security_id=security_id,
            quantity=quantity, cash_amount=cash_amount, trade_date=trade_date, settle_date=sd,
    ...
    def due(self, as_of: date) -> list:
        return [i for i in self.instructions.values()
                if i.status == SettlementStatus.PENDING and i.settle_date is not None
    ...
    def complete(self, instruction_id: str, *, as_of: date) -> SettlementRecord:
        ...
        rec = SettlementRecord(instruction_id=instruction_id, trade_id=inst.trade_id,
                               settle_date=inst.settle_date, completed_on=as_of,
                               cash_amount=inst.cash_amount, status=SettlementStatus.COMPLETED)
    ...
    def fail(self, instruction_id: str, *, reason: str = "settlement_failed") -> SettlementRecord:
        inst = self.instructions[instruction_id]
        rec = SettlementRecord(instruction_id=instruction_id, trade_id=inst.trade_id,
                               settle_date=inst.settle_date, completed_on=None,
                               cash_amount=inst.cash_amount, status=SettlementStatus.FAILED,
                               detail=reason)
        self.records[instruction_id] = rec
    ...
    def settled_cash_flow(self) -> float:
        return sum(r.cash_amount for r in self.records.values()
    ...
    def report(self, as_of: date | None = None) -> SettlementReport:
        ...
        return SettlementReport(
            as_of=as_of, n_pending=len(pend), n_completed=len(completed), n_failed=len(failed),
