...
class PostTradeEngine:
    ...
    def book_fill(
        ...
        *,
        ...
        pos_ev = PositionEvent(
            ...
            cost_basis=self._cost_basis(security_id),
            trade_id=tid,
            when=trade_date,
            reason="trade",
        )
        ...
        inst = self.settlement.instruct(
            instruction_id=f"S-{tid}",
            trade_id=tid,
            security_id=security_id,
            quantity=quantity,
            cash_amount=net_cash,
            trade_date=trade_date,
        )
        ce = CashEvent(
            seq=self._log_seq(),
            amount=net_cash,
            cash_type=CashType.TRADE,
            trade_date=trade_date,
        ...
        self.trade_ledger.record(
            TradeEvent(
                ...
                price=price,
                cost=cost,
                state=LifecycleState.SETTLEMENT_PENDING,
                trade_date=trade_date,
    ...
    def settle(self, as_of: date) -> list:
        ...
        for inst in self.settlement.due(as_of):
            ...
            self.log.emit(
                lambda seq, i=inst, r=rec: SettlementEvent(
                    seq=seq,
    ...
    def fail_settlement(self, trade_id: str, *, reason: str = "settlement_failed") -> None:
        ...
        self.log.emit(
            lambda seq: SettlementEvent(
                seq=seq,
                instruction_id=iid,
                trade_id=trade_id,
                status=SettlementStatus.FAILED,
                settle_date=rec.settle_date,
    ...
    def post_cash(
        ...
        *,
        ...
        self.accounting.state.ledger.post(
            amount, kind=cash_type.value, when=when, security_id=security_id
        )
