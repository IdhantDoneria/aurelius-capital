...
def convert(provider, amount: float, from_ccy: str, to_ccy: str, *, as_of: date | None = None,
            reason: str = "fx", conversion_id: str | None = None) -> FXConversion:
    ...
    return FXConversion(
        ...
        from_currency=from_ccy, to_currency=to_ccy, from_amount=float(amount),
        to_amount=float(amount) * rate, rate=rate, direction=direction,
        as_of=as_of, source=source, reason=reason)


def convert_to_target(provider, target_amount: float, from_ccy: str, to_ccy: str, *,
                      as_of: date | None = None, reason: str = "fx",
                      conversion_id: str | None = None) -> FXConversion:
    ...
    return convert(provider, float(target_amount) / rate, from_ccy, to_ccy, as_of=as_of,
                   reason=reason, conversion_id=conversion_id)


def round_trip_error(provider, amount: float, a: str, b: str, *, as_of: date | None = None) -> float:
...
def conversion_to_dict(c: FXConversion) -> dict:
    return {"conversion_id": c.conversion_id, "from_currency": c.from_currency,
            "to_currency": c.to_currency, "from_amount": c.from_amount, "to_amount": c.to_amount,
            "rate": c.rate, "direction": c.direction.value, "as_of": c.as_of.isoformat()
            if c.as_of else None, "source": c.source, "reason": c.reason}


def conversion_from_dict(d: dict) -> FXConversion:
    return FXConversion(
        conversion_id=d["conversion_id"], from_currency=d["from_currency"],
        to_currency=d["to_currency"], from_amount=d["from_amount"], to_amount=d["to_amount"],
        rate=d["rate"], direction=ConversionDirection(d["direction"]),
        as_of=date.fromisoformat(d["as_of"]) if d["as_of"] else None,
        source=d["source"], reason=d.get("reason", "fx"))
