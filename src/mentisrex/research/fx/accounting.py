...
def position_accounting(book, security_id: str, *, as_of: date | None = None) -> dict | None:
    ...
    return {
        ...
        "currency": ccy,
        "shares": h.shares,
        "local_cost_basis": h.cost_basis,
        "local_price": h.price,
        "local_realized_pnl": h.realized_pnl,
        "local_unrealized_pnl": h.unrealized_pnl,
        "base_cost_basis": h.cost_basis * rate,
        "base_market_value": h.market_value * rate,
        "base_unrealized_pnl": h.unrealized_pnl * rate,
        "fx_rate_to_base": rate,
    }
...
def base_realized_pnl(book, *, as_of: date | None = None) -> float:
    ...
    total = sum(
        eng.accounting.realized_pnl() * book.base_rate(ccy, as_of)
...
def base_unrealized_pnl(book, *, as_of: date | None = None) -> float:
    return sum(
        eng.accounting.unrealized_pnl() * book.base_rate(ccy, as_of)
        for ccy, eng in book.books.items()
    )
