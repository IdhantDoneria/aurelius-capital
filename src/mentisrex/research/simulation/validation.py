...
from mentisrex.backtesting.analytics.performance import EquityPoint as PerfEquityPoint
from mentisrex.backtesting.analytics.performance import PerformanceMetrics, RoundTrip
...
def validate_simulation(result, *, allow_short: bool = False, max_leverage: float = 1.05) -> dict:
    ...
    checks["cash_consistency"] = {
        "ok": cash >= -0.01 * final_value or allow_short,
        "final_cash": cash,
    }
    ...
    checks["capacity"] = {
        "signal": result.capacity_report.capacity_signal,
...
def to_performance_metrics(result) -> PerformanceMetrics:
    ...
    rts = [
        RoundTrip(
            t.security_id,
            "long" if t.quantity > 0 else "short",
            base,
    ...
    return PerformanceMetrics(
        ...
        cagr=s.cagr,
        annualized_volatility=s.volatility,
        sharpe_ratio=s.sharpe,
        sortino_ratio=s.sortino,
        max_drawdown=s.max_drawdown,
        calmar_ratio=s.calmar,
        num_trades=len(rts),
        annual_turnover=s.annualized_turnover,
        avg_holding_period_days=s.avg_holding_days,
        equity_curve=curve,
        drawdown_series=dd,
        daily_returns=rets,
        round_trips=rts,
