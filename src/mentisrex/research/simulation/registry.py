...
from mentisrex.research.simulation import serialization
...
def attach_simulation(registry, experiment, result, *, artifacts_dir: str | None = None) -> dict:
    ...
    exp.metrics = {
        **(exp.metrics or {}),
        "SimCAGR": s.cagr,
        "SimSharpe": s.sharpe,
        "SimMaxDrawdown": s.max_drawdown,
        "SimAnnualizedTurnover": s.annualized_turnover,
        "SimTotalCost": s.total_cost,
    ...
    exp.artifacts = [
        *(exp.artifacts or []),
        {
            "artifact_type": "simulation_result.json",
