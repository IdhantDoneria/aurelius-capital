...
from mentisrex.research.simulation import analytics
from mentisrex.research.simulation import attribution as attr_mod
from mentisrex.research.simulation import exposure as exp_mod
from mentisrex.research.simulation.execution import CostExecutionModel, ExecutionModel
...
from mentisrex.research.simulation.performance import build_summary, drawdown
from mentisrex.research.simulation.rebalancing import RebalancePolicy
...
class PortfolioSimulationEngine:
        ...
        *,
        ...
        for d in timeline:
            ...
            if due and tgt:
                ...
                rebals.append(
                    RebalanceEvent(d, len(orders), turned / v if v > 0 else 0.0, day_cost, "due")
                )
                total_cost += day_cost
                last = d
            ...
            snapshots.append(
                PortfolioSnapshot(
                    d, v, state.cash, w, e["gross"], e["net"], e["long"], e["short"], len(w)
                )
            )
    ...
    def _assemble(
        ...
        turn_rep = analytics.turnover_report(
            trades,
            avg_value,
            n_years=n_years,
            n_trades=len(trades),
        ...
        cost_rep = analytics.cost_report(
            trades, initial_capital=cfg.initial_capital, n_years=n_years
        )
        ...
        summary = build_summary(
            ...
            cost_drag_annualized=cost_rep.cost_drag_annualized,
        ...
        attribution = attr_mod.attribution(
            ...
            avg_cash_weight=exp_rep.avg_cash_weight,
            sectors=sectors,
        )
        ...
        meta = SimulationMetadata(
            ...
            rebalance_policy=(
                "explicit"
                if self.policy.explicit is not None
                else (self.policy.rule.mode if self.policy.rule else "every_period")
            ),
            execution_model=self.execution.name,
            cost_model={},
            allow_short=cfg.sizing.allow_short,
            config={"sizing": asdict(cfg.sizing)},
        )
        ...
        return SimulationResult(
            ...
            equity_curve=equity,
            snapshots=snapshots,
            rebalance_events=rebals,
            trades=trades,
            cost_report=cost_rep,
            turnover_report=turn_rep,
            exposure_report=exp_rep,
            drawdown_report=dd_rep,
            capacity_report=cap_rep,
            risk_timeline=risk_tl,
            attribution=attribution,
            diagnostics={
                "ledger_reconciles": state.ledger.reconciles(),
