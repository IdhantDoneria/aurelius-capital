...
def observation_from_dict(d: dict) -> CanonicalObservation:
    ...
    return CanonicalObservation(
        ...
        obs_type=ObservationType(d["obs_type"]),
        field=d["field"],
        value=d["value"],
...
def credit_curve_from_dict(d: dict) -> CreditCurve:
    return CreditCurve(
        d["curve_id"], tuple(d["tenors"]), tuple(d["hazards"]), d["recovery"], d["currency"]
    )
...
def surface_to_dict(s: VolatilitySurface) -> dict:
    return {
        ...
        "strikes": list(s.strikes),
        "maturities": list(s.maturities),
        "grid": [list(r) for r in s.grid],
...
def observations_to_json(observations, *, indent: int = 2) -> str:
    return json.dumps(
        [observation_to_dict(o) for o in observations], indent=indent, sort_keys=True, default=str
    )
