...
from mentisrex.research.market_data.sabr import calibrate_sabr
from mentisrex.research.market_data.svi import calibrate_svi
...
class VolatilitySurfaceCalibrator:
    ...
    def calibrate_smile(self, smile: SmileQuotes) -> _SmileFn:
        ...
        if self.model is VolModel.SABR:
            ...
            def fn(k, _c=cal, _f=f, _t=t):
                return _c.params.vol(_f, k, _t)
            ...
            def fn(k, _c=cal, _f=f, _t=t):
                return _c.params.vol(math.log(k / _f), _t)
    ...
    def _bid_ask_flags(self, smile: SmileQuotes, fn) -> list:
        if not smile.bids or not smile.asks:
            return []
        out = []
        for k, b, a in zip(smile.strikes, smile.bids, smile.asks, strict=False):
            v = fn(k)
    ...
    def calibrate_surface(self, smiles, surface_id: str, ref_date: date, *, strikes=None):
        ...
        for k in strikes:  # calendar-spread across expiries
            for a, b in itertools.pairwise(maturities):
        ...
        report = SurfaceCalibrationReport(
            surface_id, tuple(s.calib for s in fitted), tuple(diags), max_res
        )
...
class CalibratedVolProvider:
    ...
    def __init__(self, smiles_by_underlying: dict) -> None:
        self._smiles = {
            u: sorted(fs, key=lambda s: s.expiry) for u, fs in smiles_by_underlying.items()
        }
    ...
    def implied_vol(self, instrument_id: str, strike: float, maturity: float) -> float:
        ...
        for lo, hi in itertools.pairwise(fs):
