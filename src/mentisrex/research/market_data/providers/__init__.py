...
class ProviderMetadata:
    ...
    def __post_init__(self) -> None:
        if not self.fingerprint:
            fp = hashlib.blake2b(f"{self.name}|{self.version}".encode(), digest_size=8).hexdigest()
...
FRED = ProviderMetadata(
    ...
    limitations=(
        "us_focused",
        "optional_api_key_for_higher_limits",
...
INDIA = ProviderMetadata(
    ...
    limitations=("limited_history", "isin_required_for_dedup", "data_gov_in_irregular_updates"),
)
...
QLIB = ProviderMetadata(
    ...
    limitations=("export_only", "no_qlib_portfolio_engine", "no_qlib_execution_logic"),
)
...
FINANCETOOLKIT = ProviderMetadata(
    ...
    datasets=("profitability_ratios", "margin_ratios", "valuation_multiples", "financial_ratios"),
    limitations=(
        "derived_not_raw",
        "depends_on_sec_edgar_observations",
        "no_replace_m11_accounting",
        "no_replace_m13_risk",
