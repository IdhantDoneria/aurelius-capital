...
def deployment_readiness(
    m9_verdict: str, m9_score: float, consistency: StateConsistencyReport
) -> DeploymentReadinessReport:
    checks = {
...
def validate_session(session, *, validator=None, experiment=None) -> PaperTradingValidationResult:
    ...
    n_years = (
        max((session.equity_curve[-1].date - session.equity_curve[0].date).days / 365.25, 1e-9)
        if len(session.equity_curve) > 1
        else 1e-9
    )
    summary = build_summary(
        values or [session.config.initial_capital],
        n_rebalances=len(session.trades),
        annualized_turnover=0.0,
        avg_holding_days=0.0,
        total_cost=session.total_cost,
        cost_drag_annualized=0.0,
        periods=session.config.periods_per_year,
        n_years=n_years,
    )
