...
def to_dict(session) -> dict:
    return {
        ...
        "execution_records": [
            asdict(r) | {"when": _d(r.when), "status": r.status.value} for r in session.records
        ],
...
def _rec(r) -> dict:
    return {
        ...
        "n_external_positions": r.n_external_positions,
...
def _drift(d) -> dict:
    return {
        "as_of": _d(d.as_of),
        "max_weight_drift": d.max_weight_drift,
        "position_drift": d.position_drift,
        "cash_drift": d.cash_drift,
        "execution_drift": d.execution_drift,
        "timing_drift": d.timing_drift,
        "cost_drift": d.cost_drift,
        "alerts": d.alerts,
    }
