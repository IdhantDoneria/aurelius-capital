...
class StreamingSimulator:
    ...
    def _obs(
        self, sid, field, value, d, seq, *, source=None, ts=None, obs_date=None
    ) -> SourceMessage:
        cfg = self.config
    ...
    def _inject(self, base, faults: FaultSpec, rng) -> list[SourceMessage]:
        ...
        for m in base:
            ...
            if faults.delay_days and rng.random() < 0.5:
                ts = (msg.source_timestamp or datetime(2024, 1, 2)) + timedelta(
                    days=faults.delay_days
                )
            ...
            if (
                faults.revision_frac
                and msg.msg_type is MessageType.OBSERVATION
    ...
    def _conflicts(self, base, faults: FaultSpec, rng) -> list[SourceMessage]:
        ...
        for m in base:
            ...
            for src in faults.conflict_sources:
                payload = dict(m.payload)
                payload["value"] = round(
                    float(payload["value"]) * (1.0 + 0.02), 6
                )  # 2% disagreement
                payload["source"] = src
                out.append(
                    SourceMessage(
