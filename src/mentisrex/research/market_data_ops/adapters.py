...
class SourceAdapter(ABC):
    ...
    def health(self) -> AdapterHealthSample:
        return self._last or AdapterHealthSample(
            state=self._state, subscriptions=self.subscriptions
        )
    ...
    def _record(self, messages: list[SourceMessage]) -> list[SourceMessage]:
        ...
        if self._subscriptions:
            messages = [
                m
                for m in messages
                if m.security_hint in self._subscriptions
        ...
        self._last = AdapterHealthSample(
            ...
            if last
            else (self._last.last_sequence if self._last else None),
            last_observation_date=last.observation_date
            if last and last.observation_date
...
class LocalSourceAdapter(SourceAdapter):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            SourceMetadata(
                name or source.source,
                caps,
                schema_version,
                "M19 source wrapped as adapter",
    ...
    def fetch(self, as_of: date, *, security_ids=None, fields=None) -> list[SourceMessage]:
        ...
        for r in _stable_raw_order(raw):
            self._seq += 1
            msgs.append(
                _raw_to_message(r, self.metadata.name, self._seq, self.metadata.schema_version)
            )
...
class MessageLogAdapter(SourceAdapter):
    ...
    def __init__(
        self, messages, *, metadata: SourceMetadata | None = None, name: str = "message_log"
    ) -> None:
        super().__init__(
            metadata
            or SourceMetadata(
                name,
                frozenset((SourceCapability.HISTORICAL, SourceCapability.STREAMING)),
    ...
    def poll(self, *, max_messages: int | None = None) -> list[SourceMessage]:
        if self._state is not ConnectionState.CONNECTED:
            ...
            else min(len(self._messages), self._cursor + max_messages)
        )
...
class FixtureVendorAdapter(MessageLogAdapter):
    ...
    def __init__(self, vendor: str, messages, *, capabilities=None) -> None:
        caps = frozenset(
            capabilities
            or (SourceCapability.HISTORICAL, SourceCapability.QUOTES, SourceCapability.TRADES)
        )
        super().__init__(
            messages,
...
class ProductionSourceAdapter(SourceAdapter):
    ...
    def __init__(
        self, name: str, capabilities, *, vendor: str = "", schema_version: str = "1.0"
    ) -> None:
        super().__init__(
            SourceMetadata(
                name,
                ...
                schema_version,
                "Production adapter contract (offline)",
                vendor=vendor,
            )
        )
...
def _stable_raw_order(raw):
    ...
    def k(r):
        return (
            str(r.get("observation_date") or r.get("date") or ""),
            str(r.get("id", r.get("security_id", ""))),
            str(r.get("field", r.get("type", ""))),
        )
