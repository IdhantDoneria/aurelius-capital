...
class SequenceManager:
    ...
    def __init__(self, policy: OrderingPolicy = OrderingPolicy.REORDER, *,
                 stale_boundary: date | None = None) -> None:
        self.policy = policy
    ...
    def process(self, messages) -> OrderingReport:
        ...
        if self.policy is OrderingPolicy.REORDER:
            ...
            if [m.raw_fingerprint() for m in accepted] != [m.raw_fingerprint() for m in deduped]:
                events.append(OrderingEvent(OrderingCode.REORDERED, "", "",
                                            f"{len(accepted)} messages sorted to canonical order"))
            return OrderingReport(tuple(accepted), (), tuple(self._dropped(msgs, deduped)), tuple(events))

        if self.policy is OrderingPolicy.BUFFER:
            return OrderingReport(tuple(deduped), (), tuple(self._dropped(msgs, deduped)), tuple(events))

        if self.policy is OrderingPolicy.LATEST_VALID:
            accepted = self._latest_valid(deduped)
            return OrderingReport(tuple(self._canonical_sort(accepted)), (),
                                  tuple(self._dropped(msgs, accepted)), tuple(events))
    ...
    def _dedup(self, msgs, events) -> list[SourceMessage]:
        ...
        for m in msgs:
            ...
            if fp in seen:
                events.append(OrderingEvent(OrderingCode.DUPLICATE, fp, m.source, "identical message"))
            else:
                seen[fp] = m
        ...
        for m in msgs:
            if m.sequence is None:
                if m.msg_type not in (MessageType.HEARTBEAT, MessageType.STATUS):
                    events.append(OrderingEvent(OrderingCode.MISSING_SEQUENCE, m.raw_fingerprint(),
                                                m.source, "no sequence number"))
                continue
        ...
        for source, seq_msgs in by_source.items():
            ...
            for m in ordered:
                if m.sequence in seen_seq:
                    events.append(OrderingEvent(OrderingCode.DUPLICATE_SEQUENCE, m.raw_fingerprint(),
                                                source, f"sequence {m.sequence} repeated"))
                seen_seq.add(m.sequence)
                if prev is not None and m.sequence > prev + 1:
                    events.append(OrderingEvent(OrderingCode.SEQUENCE_GAP, m.raw_fingerprint(),
                                                source, f"gap {prev}->{m.sequence}"))
    ...
    def _diagnose_arrival_order(self, msgs, events) -> None:
        ...
        for m in msgs:
            ...
            if prev is not None and k < prev:
                events.append(OrderingEvent(OrderingCode.OUT_OF_ORDER, m.raw_fingerprint(),
                                            m.source, "arrived before an earlier-keyed message"))
    ...
    def _reject_or_quarantine(self, original, deduped, events) -> OrderingReport:
        ...
        ordered = self._canonical_sort(deduped)
        ...
        for m in deduped:
            ...
            if prev is not None and k < prev:
                offending.append(m)
                events.append(OrderingEvent(
                    OrderingCode.QUARANTINED if self.policy is OrderingPolicy.QUARANTINE
                    else OrderingCode.LATE, m.raw_fingerprint(), m.source, "out-of-order arrival"))
        ...
        if self.policy is OrderingPolicy.QUARANTINE:
            return OrderingReport(tuple(accepted), tuple(offending),
                                  tuple(self._dropped(original, deduped)), tuple(events))
        return OrderingReport(tuple(accepted), (),
                              tuple(list(self._dropped(original, deduped)) + offending), tuple(events))

    def _dropped(self, original, kept):
        keep = {id(m) for m in kept}
