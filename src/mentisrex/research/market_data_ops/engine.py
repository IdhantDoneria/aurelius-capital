...
class MarketDataOperationsEngine:
    def __init__(
        ...
        *,
        adapters=None,
        reconstructor: HistoricalReconstructor | None = None,
    ...
    # ── replay ───────────────────────────────────────────────────────────────────
    def replay(
        self, config: ReplayConfig | None = None, *, reconstruct: bool = True, **kwargs
    ) -> ReplayResult:
        engine = MarketDataReplayEngine(self._state.messages, reconstructor=self.reconstructor)
        return engine.replay(
            config, reconstruct=reconstruct, fx_provider=self.fx_provider, **kwargs
        )
    ...
    def coverage(
        self, *, expected_securities=None, expected_fields=None, expected_dates=None
    ) -> CoverageReport:
        return coverage(
            self._state.messages,
