...
from mentisrex.research.market_data_ops.lifecycle import SealedSnapshot, SnapshotState
from mentisrex.research.market_data_ops.messages import MessageType, SourceMessage
from mentisrex.research.market_data_ops.store import _envelope
...
def message_to_dict(m: SourceMessage) -> dict:
    return {
        "source": m.source,
        "payload": _clean_payload(m.payload),
...
def message_from_dict(d: dict, *, verify: bool = True) -> SourceMessage:
    m = SourceMessage(
        ...
        msg_type=MessageType(d.get("msg_type", "observation")),
...
def messages_to_json(messages, *, indent: int = 2) -> str:
    return json.dumps(
        [message_to_dict(m) for m in messages], indent=indent, sort_keys=True, default=str
    )
