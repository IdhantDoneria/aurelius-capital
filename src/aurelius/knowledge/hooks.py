...
Usage (in main.py, after KG singleton is created):
    from aurelius.knowledge import hooks
...
if TYPE_CHECKING:
    ...
    from aurelius.research.models import ExperimentRecord
...
def on_paper(paper: Paper) -> None:
    if _kg is None:
        return
    try:
        from aurelius.knowledge.ingest import _ingest_paper_obj
...
def on_hypothesis(h: HypothesisRecord) -> None:
    if _kg is None:
        return
    try:
        from aurelius.knowledge.ingest import _ingest_hypothesis_obj
...
def on_experiment(rec: ExperimentRecord) -> None:
    if _kg is None:
        return
    try:
        from aurelius.knowledge.ingest import _ingest_experiment_obj
