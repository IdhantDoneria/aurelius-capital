...
from aurelius.presentation.api.dependencies import SettingsDep
...
async def metrics(settings: SettingsDep) -> Response:
    ...
    body = (
        "# HELP aurelius_up 1 if the process is serving.\n"
        "# TYPE aurelius_up gauge\n"
        "aurelius_up 1\n"
        "# HELP aurelius_uptime_seconds Seconds since process start.\n"
        "# TYPE aurelius_uptime_seconds gauge\n"
        f"aurelius_uptime_seconds {uptime:.0f}\n"
        "# HELP aurelius_build_info Build/version labels (value always 1).\n"
        "# TYPE aurelius_build_info gauge\n"
        f'aurelius_build_info{{version="{_VERSION}",'
