...
def sabr_vol(f: float, k: float, t: float, alpha: float, beta: float, rho: float, nu: float) -> float:
    ...
    if abs(f - k) < 1e-9 * max(f, 1.0):                        # ATM
        fk_beta = f ** one_beta
        term = (((one_beta ** 2) / 24.0) * alpha ** 2 / (fk_beta ** 2)
    ...
    if abs(xz) < eps:
        xz = eps
    denom = fk_beta * (1.0 + (one_beta ** 2) / 24.0 * logfk ** 2
...
def calibrate_sabr(f: float, t: float, strikes, market_vols, *, beta: float = 0.5,
                   refine: bool = True) -> SABRCalibration:
    ...
    atm = min(zip(strikes, market_vols), key=lambda kv: abs(kv[0] - f))[1]
    ...
    resid = tuple((k, sabr_vol(f, k, t, alpha, beta, rho, nu) - mv)
                  for k, mv in zip(strikes, market_vols))
