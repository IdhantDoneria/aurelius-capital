...
from aurelius.research.valuation.curves import ZeroCurve
from aurelius.research.valuation.registry import CalibrationDiagnostics, CurveCalibrationReport
...
class CreditCurve:
    ...
    def _cum_hazard(self, t: float) -> float:
        if t <= 0:
            return 0.0
        total, prev = 0.0, 0.0
        for ti, hi in zip(self.tenors, self.hazards):
            seg = min(t, ti) - prev
            if seg > 0:
                total += hi * seg
            prev = ti
    ...
    def fingerprint(self) -> str:
        ...
        parts += [f"{t:.6g}:{h:.10g}" for t, h in zip(self.tenors, self.hazards)]
...
def _cds_pv(spread, tenor, freq, hazards_tenors, hazards, disc: ZeroCurve, recovery, ref):
    ...
    for j in range(1, n + 1):
        ...
        prev_surv, prev_t = surv, t
    return prem - prot


def bootstrap_credit(quotes, disc: ZeroCurve, *, recovery: float = 0.4, curve_id: str = "credit",
                     currency: str = "USD", ref=None, tol: float = 1e-12, strict: bool = True):
    ...
    for q in qs:
        def f(h, _q=q):
            return _cds_pv(_q.spread, _q.tenor, _q.frequency, tenors + [_q.tenor],
                           hazards + [h], disc, recovery, ref)
    ...
    residuals = tuple(
        (f"CDS@{q.tenor:g}y",
         _cds_pv(q.spread, q.tenor, q.frequency, curve.tenors, curve.hazards, disc, recovery, ref))
        for q in qs)
    ...
    report = CurveCalibrationReport(
        curve_id, CalibrationDiagnostics(max_res, len(qs), not problems),
        tuple(n for n, _ in residuals), problems)
