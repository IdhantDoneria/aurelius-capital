...
from aurelius.research.market_data.models import QualityStatus, Severity
from aurelius.research.market_data.normalization import NormalizationResult, Normalizer
from aurelius.research.market_data.quality import MarketDataQualityEngine, QualityConfig
from aurelius.research.valuation.models import MarketDataSnapshot, MarketQuote, Provenance
...
class MarketDataSnapshotBuilder:
    def __init__(self, *, normalizer: Normalizer | None = None,
                 quality: MarketDataQualityEngine | None = None, source: str = "m19") -> None:
        self.normalizer = normalizer or Normalizer()
        self.quality = quality or MarketDataQualityEngine()
        self.source = source

    def build(self, *, as_of: date, raw: list[dict] | None = None, source=None,
              security_ids=None, fields=None, curves=None, discount_curves=None,
              vol_surfaces=None, dividend_yields=None, forwards=None, fx_provider=None,
              corporate_actions=None, policy: PITPolicy | None = None) -> BuildResult:
        ...
        qcfg = QualityConfig(max_staleness_days=policy.max_staleness_days,
        ...
        if policy.fail_closed:
            for obs in qrep.rejected:
                if obs.field in ("close", "last") or obs.obs_type.value in ("close", "trade"):
                    crit = next((d for d in qrep.diagnostics
                                 if d.security_id == obs.security_id and d.rejects), None)
        ...
        snap = MarketDataSnapshot(
            as_of=as_of, spots=spots, quotes=quotes, rates=dict(curves or {}),
            discount_factors=dict(discount_curves or {}), vol_surfaces=dict(vol_surfaces or {}),
            dividend_yields=dict(dividend_yields or {}), forwards=dict(forwards or {}),
            corporate_actions=dict(corporate_actions or {}), fx_provider=fx_provider,
        ...
        return BuildResult(snap, tuple(diags), tuple(qrep.accepted), norm.transform_log,
                           snap.fingerprint())
    ...
    def _assemble_spots(self, observations):
        ...
        for o in observations:
            if o.field in ("close", "last"):
                ...
                if cur is None or (o.revision, o.observation_date) >= (cur.revision, cur.observation_date):
        ...
        for sid in set(bids) | set(asks):
            quotes[sid] = MarketQuote(
                instrument_id=sid, value=spots.get(sid, 0.5 * (bids.get(sid, 0.0) + asks.get(sid, 0.0))),
                currency=(best_close.get(sid).currency if best_close.get(sid) else "USD") or "USD",
                bid=bids.get(sid), ask=asks.get(sid),
