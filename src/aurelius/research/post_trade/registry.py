...
from aurelius.research.post_trade import reporting, serialization
from aurelius.research.post_trade.diagnostics import fingerprint as _fingerprint
...
def attach_post_trade(registry, experiment, engine, *, artifacts_dir: str | None = None) -> dict:
    ...
    exp.notes = (f"post_trade trades={rep.ledger.n_trade_events} "
    ...
    exp.artifacts = [*(exp.artifacts or []),
                     {"artifact_type": "post_trade_session.json", "artifact_location": str(path),
                      "artifact_hash": h}]
