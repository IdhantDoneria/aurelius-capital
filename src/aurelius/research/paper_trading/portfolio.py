...
from aurelius.research.paper_trading.models import AccountSnapshot, BrokerFill, PositionSnapshot
...
class PaperPortfolio:
    ...
    def ingest_fill(self, fill: BrokerFill) -> bool:
        ...
        self.state.apply_fill(fill.security_id, fill.quantity, fill.price, fill.cost, when=fill.when)
        return True
    ...
    def snapshot(self, external, when=None) -> AccountSnapshot:
        ...
        return AccountSnapshot(
            date=when, internal_cash=self.cash,
            external_cash=external.cash if external else 0.0,
            internal_value=self.value(),
            external_value=external.total_value() if external else 0.0, positions=rows)
