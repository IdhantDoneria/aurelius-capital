...
from aurelius.research.simulation.state import CashLedger, PortfolioState
...
def _deser_cycle_records(data: list) -> list:
    from aurelius.research.paper_trading.cycle import CycleRecord
...
def _deser_sync_events(data: list) -> list:
    from aurelius.research.paper_trading.models import SyncEvent
    events = []
    for d in data:
        dt = d.get("date")
