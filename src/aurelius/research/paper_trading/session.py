...
from aurelius.research.simulation.models import EquityPoint, Trade
...
class PaperTradingSession:
    def __init__(self, *, broker, config: SessionConfig | None = None,
                 risk_gate: PreTradeRiskGate | None = None, cost_model=None) -> None:
        self.broker = broker
    ...
    def step(self, when: date, target: dict, prices: dict, *, adv_provider=None) -> SyncEvent:
        ...
        for o in approved:
            ...
            bo = self.broker.place_order(
                OrderRequest(coid, o.security_id, o.quantity), adv=adv)
            self.records.append(ExecutionRecord(
                coid, bo.broker_order_id, o.security_id, o.quantity,
                bo.filled_quantity, bo.avg_fill_price, 0.0, bo.status, when))
        ...
        for f in fills:
            ...
            if self.book.ingest_fill(f):
                ...
                self.trades.append(Trade(f.security_id, f.quantity, f.price, f.cost,
                                         f.quantity * f.price, date=when))
                day_cost += f.cost
        ...
        pending = [(r.client_order_id, self._age(r.when, when))
                   for r in self.records if r.status in (OrderStatus.PENDING, OrderStatus.SUBMITTED)]
        rec = reconcile(self.book.state, external, when=when, config=cfg.reconciliation,
                        pending_orders=pending, applied_fill_ids=self._applied_fill_ids,
        ...
        drift = compute_drift(self.book.state, external, target or {}, when=when,
                              timing_gap_days=timing_gap, execution_bps=exec_bps,
                              expected_cost=expected_cost, actual_cost=day_cost,
        ...
        ev = SyncEvent(seq=len(self.sync_events), date=when, n_orders=len(approved),
                       n_fills=len(fills), reconciled=rec.ok, n_drift_alerts=len(drift.alerts),
                       note="; ".join(drift.alerts) if drift.alerts else ("ok" if rec.ok else "break"))
        self.sync_events.append(ev)
        return ev

    def run(self, timeline, target_provider, price_provider, *, adv_provider=None) -> list[SyncEvent]:
        for d in timeline:
    ...
    def fingerprint(self) -> str:
        ...
        body = "|".join(f"{s.date}:{s.n_orders}:{s.n_fills}:{int(s.reconciled)}"
