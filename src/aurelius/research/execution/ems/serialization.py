...
from aurelius.research.execution.ems import monitoring


def to_dict(session) -> dict:
    from aurelius.research.execution.ems.diagnostics import diagnostics as _diag
    return {
        ...
        "rejections": [{"order_id": o, "security_id": s, "quantity": q, "reason": r}
...
def _report(r) -> dict:
    return {
        "order_id": r.order_id, "security_id": r.security_id,
        "requested_quantity": r.requested_quantity, "filled_quantity": r.filled_quantity,
        "avg_fill_price": r.avg_fill_price, "arrival_price": r.arrival_price,
        "total_cost": r.total_cost, "status": r.status.value,
        "slippage_bps": r.slippage_bps, "implementation_shortfall_bps": r.implementation_shortfall_bps,
        "fill_rate": r.fill_rate, "n_child_orders": r.n_child_orders, "n_fills": r.n_fills,
        "events": [_event(e) for e in r.events],
    }


def _event(e) -> dict:
    return {"seq": e.seq, "order_id": e.order_id, "kind": e.kind, "status": e.status.value,
            "detail": e.detail, "filled_quantity": e.filled_quantity,
            "when": e.when.isoformat() if e.when else None}


def _fill(f) -> dict:
    return {"fill_id": f.fill_id, "order_id": f.order_id, "child_order_id": f.child_order_id,
            "security_id": f.security_id, "quantity": f.quantity, "price": f.price,
            "cost": f.cost, "when": f.when.isoformat() if f.when else None}
