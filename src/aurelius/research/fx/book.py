...
from aurelius.research.fx import conversion as _conv
from aurelius.research.fx.currency import CurrencyMismatchError, validate_code
...
from aurelius.research.post_trade.settlement import SettlementConfig


class MultiCurrencyBook:
    def __init__(self, base_currency: str, provider, *, initial: dict | None = None,
                 settlement_config: SettlementConfig | None = None,
                 session_id: str = "fx_book") -> None:
    ...
    def _book(self, ccy: str, *, initial: float = 0.0) -> PostTradeEngine:
        ...
        if eng is None:
            eng = PostTradeEngine(initial, settlement_config=self.settlement_config,
    ...
    def base_rate(self, ccy: str, when: date | None = None) -> float:
        ...
        ccy = validate_code(ccy)
        return 1.0 if ccy == self.base_currency else self.provider.rate(
            ccy, self.base_currency, as_of=when)

    # ── trading ────────────────────────────────────────────────────────────────
    def book_fill(self, *, security_id: str, quantity: float, price: float, cost: float = 0.0,
                  currency: str, funding_currency: str | None = None,
        ...
        if prev and prev != currency:
            raise CurrencyMismatchError(
                f"{security_id} trades in {prev}, got {currency}")
        ...
        if funding_currency and validate_code(funding_currency) != currency and quantity > 0:
            self.convert(needed_to=quantity * price + cost, from_currency=funding_currency,
                         to_currency=currency, when=trade_date, reason="trade_funding")

        return eng.book_fill(security_id=security_id, quantity=quantity, price=price, cost=cost,
                             trade_date=trade_date, fill_id=fill_id, trade_id=trade_id)

    # ── FX conversion ──────────────────────────────────────────────────────────
    def convert(self, *, amount: float | None = None, needed_to: float | None = None,
                from_currency: str, to_currency: str, when: date | None = None,
                settled: bool = True, reason: str = "fx"):
        ...
        if needed_to is not None:
            fxc = _conv.convert_to_target(self.provider, needed_to, from_ccy, to_ccy,
                                          as_of=when, reason=reason, conversion_id=cid)
        else:
            fxc = _conv.convert(self.provider, amount, from_ccy, to_ccy, as_of=when,
                                reason=reason, conversion_id=cid)
    ...
    def post_cash(self, currency: str, amount: float, cash_type: CashType, *,
                  when: date | None = None, security_id: str | None = None,
                  settled: bool = True) -> int:
        return self._book(currency).post_cash(amount, cash_type, when=when,
                                              security_id=security_id, settled=settled)
