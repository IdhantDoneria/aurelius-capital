...
from aurelius.research.risk.models import StressResult, StressScenario, StressTestReport

# Canonical historical scenarios (approximate peak-to-trough equity shocks).
HISTORICAL_SCENARIOS = {
    "gfc_2008": StressScenario("gfc_2008", market_shock=-0.50, vol_multiplier=2.5,
                               liquidity_multiplier=0.4),
    "covid_2020": StressScenario("covid_2020", market_shock=-0.34, vol_multiplier=3.0,
                                 liquidity_multiplier=0.5),
    "inflation_2022": StressScenario("inflation_2022", market_shock=-0.25, vol_multiplier=1.6,
                                     liquidity_multiplier=0.7),
}


def apply_scenario(weights: dict, scenario: StressScenario, *, value: float = 1.0,
                   sectors: dict | None = None, betas: dict | None = None,
                   halt_threshold: float = -0.20) -> StressResult:
    ...
    for sid, w in (weights or {}).items():
        ...
        pnl += w * shock
    return StressResult(scenario=scenario.name, pnl_fraction=float(pnl),
                        stressed_value=float(value * (1 + pnl)),
                        breached=bool(pnl <= halt_threshold))


def stress_test(weights: dict, *, scenarios=None, value: float = 1.0, sectors=None,
                betas=None, halt_threshold: float = -0.20) -> StressTestReport:
    scen = scenarios if scenarios is not None else list(HISTORICAL_SCENARIOS.values())
    results = [apply_scenario(weights, s, value=value, sectors=sectors, betas=betas,
                              halt_threshold=halt_threshold) for s in scen]
