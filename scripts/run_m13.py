#!/usr/bin/env python
"""M13 long-only low-volatility runs — one config per invocation.

Independent hypothesis from M12: does a LONG-ONLY low-vol portfolio (no short leg)
produce statistically significant and economically deployable alpha? Everything
frozen vs M12 except allow_short=False — same volatility definition, M8 bounded
construction, monthly rebalance, $5 screen, US canonical panel.

mode=full  : investigate (IS/OOS + adjusted p, certified framework) AND continuous.
mode=cont  : continuous full-sample run_backtest only (faster) → robustness.

    python scripts/run_m13.py <label> <mode> <outfile.jsonl>
"""
...
# LONG-ONLY: allow_short=False is the only change from the M12 baseline.
...
CONFIGS = {
    ...
    # Phase 3 robustness (single pre-registered values, no sweep-to-optimize)
...
def main() -> None:
    ...
    print(f"[m13:{label}] {len(bars)} bars {len({b.symbol for b in bars})} names "
    ...
    if mode == "full":
        store = ResearchStore(f"./data/research_m13_{label}.duckdb")
        runner = ResearchRunner(store)
        h = runner.hypothesis(
            statement="M13 long-only low-vol: a long-only lowest-volatility-decile "
                      "portfolio (no short leg) earns statistically significant, "
                      "economically deployable risk-adjusted alpha.",
            rationale="Baker-Haugen-Baker / Blitz-van Vliet total-volatility anomaly; "
                      "252d stdev of daily returns, long lowest decile, monthly, M8 "
                      "bounded construction. M12 localized L/S failure to the short leg.",
            researcher="m13_longonly_lowvol_campaign")
    ...
    print(f"[m13:{label}] {json.dumps(rec)}", flush=True)
