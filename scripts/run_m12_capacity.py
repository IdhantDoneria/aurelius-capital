#!/usr/bin/env python
"""M12 Phase 7 — low-vol capacity scaling (analytic, India ₹).

Capacity is bound by the LEAST-LIQUID held name. Low-vol holds the low-volatility
decile (long) and high-volatility decile (short); their liquidity profiles differ
from a momentum decile, so capacity is computed over each leg's own ₹-ADV.

Rank India (.NS) names by trailing 252d stdev of daily returns; take the low-vol
decile (long) and high-vol decile (short). Per-name weight = M8 invariant weight.
For each ₹ size: position = size × weight; ADV% = position ÷ 21d median ₹-volume.
Report the p10 (least-liquid) name's ADV% per leg and the capacity ceiling (≤10% ADV).

    python scripts/run_m12_capacity.py
Output: campaign/lowvol/capacity_india.json
"""
...
def load() -> dict[str, tuple[float, float]]:
    ...
    for sym, cv in by.items():
        if len(cv) < need or cv[-1][0] < MIN_PRICE:  # last (rn=1) is latest close
            continue
...
def main() -> None:
    out = Path("campaign/lowvol/capacity_india.json")
    ...
    low = ranked[:decile]        # low-vol decile (long)
    high = ranked[-decile:]      # high-vol decile (short)
    print(f"[capacity] India {n} names  decile {decile}  weight {weight:.4f}")
    low_r = leg_report("low-vol (long)", [adv for _, (_, adv) in low], weight)
    high_r = leg_report("high-vol (short)", [adv for _, (_, adv) in high], weight)
    out.write_text(json.dumps({"universe_n": n, "decile_count": decile,
        "per_name_weight": weight, "participation_cap": CAP,
        "low_vol_long": low_r, "high_vol_short": high_r}, indent=2) + "\n")
    print(f"Output: {out}")
