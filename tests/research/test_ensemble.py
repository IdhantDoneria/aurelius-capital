...
def test_inverse_var_downweights_noisy_factor():
    ...
    sm = {"calm": list(rng.standard_normal(300) * 0.005),
...
def test_correlation_matrix_shape():
    names, corr = correlation_matrix(_indep(3, 100))
...
def test_combine_from_campaign_library():
    ...
    def factor(edge, seed):
        ...
        for _ in range(80):
            ...
            sig.append(dict(zip(names, s)))
            fwd.append(dict(zip(names, edge * s + rng.standard_normal(50))))
        return sig, fwd
