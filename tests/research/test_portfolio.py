...
def test_equal_weight(engine):
    p = engine.construct(
        _signals(), IDS, ConstraintSet(), Objective.EQUAL_WEIGHT, covariance=_cov()
    )
    w = [pos.weight for pos in p.positions]
    assert all(abs(x - 1.0 / N) < 1e-9 for x in w)
    assert abs(p.gross_exposure - 1.0) < 1e-9
    assert p.n_positions == N
...
def test_turnover(engine):
    current = dict.fromkeys(IDS, 1.0 / N)
    p = engine.construct(
        _signals(),
        IDS,
        ConstraintSet(),
    ...
    p2 = engine.construct(
        _signals(),
        IDS,
        ConstraintSet(max_position_weight=0.15),
...
def test_long_only(engine):
    p = engine.construct(
        _signals(), IDS, ConstraintSet(long_only=True), Objective.MAX_SHARPE, covariance=_cov()
    )
...
def test_leverage(engine):
    c = ConstraintSet(
        long_only=False, gross_exposure=1.5, max_leverage=1.5, max_position_weight=0.3
    )
    p = engine.construct(_signals(), IDS, c, Objective.MAX_SHARPE, covariance=_cov())
...
def test_liquidity(engine):
    ...
    p = engine.construct(
        ...
        current_weights=current,
        cost_model=cm,
        adv=dict.fromkeys(IDS, 100000.0),
        capital=1e8,
    )
...
def test_execution_integration():
    from mentisrex.backtesting.analytics.performance import EquityPoint, PerformanceMetrics
    from mentisrex.research.execution import ResearchRunner, RunConfiguration

    reg = ExperimentRegistry(store=RegistryStore(":memory:"))
    try:
        curve = [
            EquityPoint(datetime(2020, 1, 1, tzinfo=UTC), 1e6),
        ...
        p = PortfolioEngine().construct(
            _signals(),
            IDS,
            ConstraintSet(max_position_weight=0.2),
            Objective.EQUAL_WEIGHT,
            covariance=_cov(),
        )
        record_portfolio(
        ...
        assert reg.load(session.experiment_id).metrics.get(
...
def test_validation_and_matrix_integration(engine):
    frame = pd.DataFrame(
        {"market_cap": np.linspace(1, 2, N), "leverage": np.linspace(2, 1, N)}, index=IDS
    )
    matrix = ResearchMatrix(
        as_of_date=date(2020, 6, 30),
        universe_size=N,
        data_versions={},
        generated_at=datetime(2020, 6, 30, tzinfo=UTC),
        frame=frame,
    ...
    p = engine.construct(
        sig, IDS, ConstraintSet(max_position_weight=0.15), Objective.MAX_SHARPE, covariance=_cov()
    )
    checks = validate_portfolio(
        p, ConstraintSet(max_position_weight=0.15, max_turnover=1.0), cost={"total_cost_bps": 12.0}
    )
    assert checks["ok"]
    assert checks["concentration"]["ok"]
