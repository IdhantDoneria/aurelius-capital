...
from mentisrex.research.experiment_registry import (
    ExperimentRegistry,
    RegistryStore,
    check,
    hashing,
...
def _start(reg, name="exp", params=None, features=None, seed=42):
    return reg.start_experiment(
        name,
        parameters=params or {"lookback": 252, "top": 50},
        features=features or ["market_cap", "roe"],
...
def test_parameter_hash_order_independent():
    assert hashing.hash_params({"lookback": 252, "top": 50}) == hashing.hash_params(
        {"top": 50, "lookback": 252}
    )
    # nested dicts too
    assert hashing.hash_params({"a": {"x": 1, "y": 2}}) == hashing.hash_params(
        {"a": {"y": 2, "x": 1}}
    )
...
def test_duplicate_detected(reg):
    ...
    dup = reg.start_experiment(
        "exp2",
        parameters={"top": 50, "lookback": 252},
        features=["roe", "market_cap"],
        dataset_versions=_dv(),
        random_seed=7,
    )
    ...
    solo = reg.start_experiment(
        "exp3", parameters={"lookback": 100}, features=["roe"], dataset_versions=_dv()
    )
...
def test_search(reg):
    ...
    assert by_name[0].experiment_id == a.experiment_id
...
def test_failure_path(reg):
    ...
    ok = reg.start_experiment(
        "recovery", parameters={"lookback": 10}, features=["close"], dataset_versions=_dv()
    )
    assert reg.load(ok.experiment_id).status == "running"
    # quality flags the incomplete failed run without raising
    rep = check(reloaded)
    assert rep["experiment_id"] == exp.experiment_id
