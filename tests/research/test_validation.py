...
def _pm(drift=0.0012, vol=0.008, n=750, turnover=2.0, seed=1):
    ...
    rts = [
        RoundTrip("A", "long", t0, t0 + timedelta(days=5), 100, 100.0, 110.0, 900.0),
    ...
    return PerformanceMetrics(
        ...
        sharpe_ratio=2.0,
        max_drawdown=min(d for _, d in dd),
        num_trades=2,
        annual_turnover=turnover,
        avg_holding_period_days=6.5,
        equity_curve=curve,
        drawdown_series=dd,
        daily_returns=rets,
        round_trips=rts,
...
def _validator(**over):
    cfg = ValidationConfig(
        bootstrap_samples=300, monte_carlo_samples=200, permutation_samples=400, n_trials=1, **over
    )
...
def test_bootstrap_correctness():
    ...
    assert ci["ci_low"] == ci2["ci_low"]
    assert ci["ci_high"] == ci2["ci_high"]
    # all four methods run
    for m in bootstrap.METHODS:
        assert (
            bootstrap.bootstrap_ci(r, sharpe, n_samples=100, method=m, seed=1)["n_samples"] == 100
        )
...
def test_parameter_sensitivity():
    ...
    assert 0.0 <= stab["plateau_score"] <= 1.0
    assert stab["peak"] > 0
...
def test_capacity_estimation():
    ...
    assert with_adv["adv_supplied"]
    assert with_adv["implementation_shortfall"] >= 0
    assert with_adv["estimated_capacity_aum"] > 0
    without = cap_mod.capacity_analysis(pm, adv=None)
    assert without["adv_supplied"] is False
    assert "capacity_signal" in without
...
def test_multiple_testing():
    ...
    assert all(a >= p for a, p in zip(bonf["adjusted"], pvals, strict=False))
    holm = multiple_testing.holm(pvals)
    bh = multiple_testing.benjamini_hochberg(pvals)
    assert holm["reject"][0]
    assert bh["reject"][0]
...
def test_verdict_logic():
    ...
    assert weak.overall_verdict == "REJECT"
    assert weak.critical_failures
...
def test_registry_integration(registry, tmp_path):
    ...
    v = ResearchValidator(
        config=ValidationConfig(
            bootstrap_samples=200, monte_carlo_samples=100, permutation_samples=200, n_trials=1
        ),
        registry=registry,
    )
...
def test_execution_integration(registry, tmp_path):
    ...
    v = ResearchValidator(
        config=ValidationConfig(
            bootstrap_samples=150, monte_carlo_samples=80, permutation_samples=150, n_trials=1
        ),
        registry=registry,
    )
...
def test_scoring_weights_configurable():
    summaries = {
        "significance": significance(_series(0.001, 0.01, 400)),
        "overfitting": {"dsr": 0.9},
        "robustness": {
            "rolling": {"share_positive": 0.8},
            "expanding": {"share_positive": 0.8},
            "missing_data": {"max_degradation": 0.0},
        },
