...
def _panels(T, N, edge, adv_level, seed=0):
    ...
    for _ in range(T):
        ...
        signals.append(dict(zip(names, s)))
        fwd.append(dict(zip(names, edge * s + rng.standard_normal(N) * 0.05)))
        adv.append({n: adv_level for n in names})
...
def test_sharpe_decays_with_aum():
    ...
    r = capacity_curve(sig, fwd, adv, cost_model=cm,
                       aum_levels=[1e6, 1e7, 1e8, 1e9])
...
def test_higher_adv_gives_more_capacity():
    cm = TransactionCostModel(impact_coef=0.1)
    lo = capacity_curve(*_panels(80, 50, 0.02, 1e5), cost_model=cm,
                        aum_levels=[1e7, 1e8])
    hi = capacity_curve(*_panels(80, 50, 0.02, 1e8), cost_model=cm,
                        aum_levels=[1e7, 1e8])
...
def test_half_capacity_detected():
    ...
    r = capacity_curve(sig, fwd, adv, cost_model=cm,
                       aum_levels=[1e6, 1e7, 5e7, 1e8, 5e8, 1e9, 5e9])
    if r["base_sharpe"] > 0:
