...
def test_book_fills_batch():
    ...
    class F:
        def __init__(s, sid, q, p):
            s.security_id, s.quantity, s.price, s.cost, s.fill_id, s.when = sid, q, p, 0.0, f"f{sid}", MON
    ids = eng.book_fills([F("AAA", 10, 100.0), F("BBB", 5, 50.0)])
    assert len(ids) == 2 and eng.accounting.shares("BBB") == pytest.approx(5)
...
def test_settle_completes_due():
    ...
    assert done == [tid] and eng.trades[tid] == LifecycleState.SETTLED
...
def test_settlement_report_counts():
    ...
    assert rep.n_completed == 2 and rep.n_pending == 0
...
def test_cash_available_is_settled():
    eng = _eng()
    _booked(eng, qty=100, price=100.0)
    assert eng.cash_ledger.available() == pytest.approx(1_000_000)   # unsettled outflow not yet available
    eng.settle(WED)
...
def test_cash_dividend():
    eng = _eng()
    _booked(eng, qty=100)
    PT.apply_corporate_action(eng, DividendEvent(action_id="D", security_id="AAA",
                                                 amount_per_share=2.0, ex_date=WED))
...
def test_cash_dividend_reconciles():
    eng = _eng()
    _booked(eng, qty=100)
    PT.apply_corporate_action(eng, DividendEvent(action_id="D", security_id="AAA", amount_per_share=2.0))
...
def test_stock_dividend_adds_shares():
    eng = _eng()
    _booked(eng, qty=100, price=100.0)
    PT.apply_corporate_action(eng, DividendEvent(action_id="SD", security_id="AAA", stock_ratio=0.1))
...
def test_split_doubles_shares_halves_basis():
    ...
    assert h.shares == pytest.approx(200) and h.cost_basis == pytest.approx(50)
...
def test_merger_cash_and_rename():
    eng = _eng()
    _booked(eng, qty=100, price=100.0)
    PT.apply_corporate_action(eng, MergerEvent(action_id="M", security_id="AAA",
                                               new_security_id="BBB", share_ratio=1.0, cash_per_share=5.0))
    assert eng.accounting.shares("AAA") == 0 and eng.accounting.shares("BBB") == pytest.approx(100)
...
def test_merger_ratio_keeps_ledger_reconciled():
    eng = _eng()
    _booked(eng, qty=100, price=100.0)
    PT.apply_corporate_action(eng, MergerEvent(action_id="M2", security_id="AAA",
                                               new_security_id="CCC", share_ratio=1.5))
...
def test_symbol_change_keeps_ledger_reconciled():
    eng = _eng()
    _booked(eng, qty=100)
    PT.apply_corporate_action(eng, SymbolChangeEvent(action_id="SC2", security_id="AAA", new_security_id="ZZZ"))
    assert PT.ledger_reconciles(eng)
...
def test_symbol_change():
    eng = _eng()
    _booked(eng, qty=100)
    PT.apply_corporate_action(eng, SymbolChangeEvent(action_id="SC", security_id="AAA", new_security_id="ZZZ"))
    assert eng.accounting.shares("ZZZ") == pytest.approx(100) and eng.accounting.shares("AAA") == 0


def test_delisting_closes_position():
    eng = _eng()
    _booked(eng, qty=100, price=100.0)
    PT.apply_corporate_action(eng, DelistingEvent(action_id="DL", security_id="AAA", final_price=0.0))
...
def test_corporate_action_report():
    ...
    assert rep.n_actions == 2 and rep.total_cash_impact == pytest.approx(100)
...
def test_reconcile_incorrect_quantity():
    ...
    assert not rep.ok and any(d["category"] == "incorrect_quantity" for d in rep.differences)
...
def test_reconcile_missing_trade():
    ...
    assert not rep.ok and any(d["category"] == "missing_trade" for d in rep.differences)
...
def test_book_m14_execution_report():
    ...
    sess = engine.execute([E.market_order("o", "AAA", 100, arrival_price=100.0)],
                          MarketInfo(prices={"AAA": 100.0}))
    pt = _eng()
    tid = pt.book_execution_report(sess.reports()[0], trade_date=MON)
    assert tid and pt.accounting.shares("AAA") == pytest.approx(100)
...
def test_book_m14_fills():
    ...
    assert len(ids) == 4 and pt.accounting.shares("AAA") == pytest.approx(100)
...
def test_post_trade_report_composite():
    ...
    assert rep.health.ok and rep.ledger.reconciles and rep.cash.reconciles
...
def test_operational_health_flags_failure():
    ...
    assert not h.ok and h.n_failed_settlements == 1
...
def test_serialization_has_events():
    ...
    assert d["events"] and d["report"] and "diagnostics" in d
...
def test_fingerprint_changes_with_content():
    e1 = _eng(); _booked(e1, qty=100)
    e2 = _eng(); _booked(e2, qty=50)
...
def test_replay_reproduces_event_count():
    ...
    assert seen == sorted(seen) and len(seen) == len(eng.log)
...
def test_registry_attach(tmp_path):
    ...
    assert "PostTradeValue" in reg.store.rows["E1"].metrics and out["hash"]
...
def test_empty_engine_reports():
    ...
    assert rep.n_positions == 0 and rep.health.ok
...
def test_delisting_nonzero_price_books_cash():
    eng = _eng()
    _booked(eng, qty=100, price=100.0)
    PT.apply_corporate_action(eng, DelistingEvent(action_id="DL2", security_id="AAA", final_price=30.0))
...
def test_stock_dividend_keeps_ledger_reconciled():
    eng = _eng()
    _booked(eng, qty=100)
    PT.apply_corporate_action(eng, DividendEvent(action_id="SD2", security_id="AAA", stock_ratio=0.5))
