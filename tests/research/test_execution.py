...
def _pm(pnl1=1000.0, pnl2=-500.0):
    ...
    rts = [RoundTrip("AAA", "long", t0, t0 + timedelta(days=5), 100, 100.0, 110.0, pnl1),
           RoundTrip("BBB", "long", t0, t0 + timedelta(days=3), 100, 50.0, 45.0, pnl2)]
    return PerformanceMetrics(
        total_return=0.03, cagr=0.4, annualized_volatility=0.1, sharpe_ratio=1.5,
        sortino_ratio=2.0, max_drawdown=-0.05, calmar_ratio=8.0, num_trades=2, win_rate=0.5,
        avg_holding_period_days=4.0, annual_turnover=1.2, profit_factor=2.0,
...
def _cfg(tmp_path, name="exp", executor=fake_executor, **over):
    kw = dict(name=name, parameters={"lookback": 252}, features=["market_cap", "roe"],
              dataset_versions=_dv(), random_seed=42, executor=executor,
...
def test_failure_recovery(runner, tmp_path):
    ...
    assert "ValueError" in s.error and s.traceback
    ...
    d = tmp_path / "boom"
    assert (d / "traceback.txt").exists() and (d / "execution_log.json").exists()
...
def test_state_transitions(runner, tmp_path):
    s = runner.run(_cfg(tmp_path))
    trans = [e.data["from_state"] + "->" + e.stage
             for e in s.events.by_name("state_transition")]
    assert trans == [
        "CREATED->VALIDATING", "VALIDATING->BUILDING_MATRIX", "BUILDING_MATRIX->RUNNING",
        "RUNNING->GENERATING_METRICS", "GENERATING_METRICS->WRITING_ARTIFACTS",
        "WRITING_ARTIFACTS->FINALIZING", "FINALIZING->COMPLETED",
    ]
...
def test_event_logging(runner, tmp_path):
    ...
    for expected in ("registry_started", "validation_passed", "execution_completed",
                     "metrics_completed", "artifacts_written", "registry_updated", "run_completed"):
...
def test_matrix_integration(tmp_path):
    ...
    assert s.state == State.COMPLETED
    assert s.matrix is not None and s.matrix.universe_size == 3
...
def test_resume(runner, tmp_path):
    ...
    assert s.state == State.VALIDATING and s.report is None   # paused
    resumed = runner.resume(s)
    assert resumed.state == State.COMPLETED and resumed.report is not None
...
def test_validation_failures(runner, tmp_path):
    ...
    s = runner.run(bad)
    assert s.state == State.FAILED and s.report is None
...
def test_real_backtest_through_platform(runner, tmp_path):
    ...
    executor = make_backtest_executor(MeanReversionStrategy(lookback=20),
                                      InMemoryDataFeed(bars), research_config())
    s = runner.run(_cfg(tmp_path, name="real", executor=executor,
                        parameters={"lookback": 20}))
    assert s.state == State.COMPLETED
    assert "Sharpe" in s.metrics and "MaxDrawdown" in s.metrics
    assert check(s)["ok"]
