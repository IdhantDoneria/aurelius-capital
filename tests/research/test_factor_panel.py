...
def _matrix(d, values, feature="mom", direction="higher"):
    frame = pd.DataFrame({feature: values},
                         index=[f"sid{i}" for i in range(len(values))])
...
def test_builds_aligned_panels_and_drops_last_date():
    ...
    ms = [_matrix(d1, [1.0, 2.0, 3.0]), _matrix(d2, [3.0, 2.0, 1.0]),
          _matrix(d3, [1.0, 1.0, 1.0])]
...
def test_missing_forward_price_dropped():
    ...
    assert "sid0" in fwd[0] and "sid1" not in fwd[0]


def test_needs_two_matrices():
    with pytest.raises(ValueError):
        panels_from_matrices([_matrix(date(2020, 1, 1), [1.0])],
                             "mom", close_fn=lambda *a, **k: 1.0, symbol_fn=lambda *a: "x")
