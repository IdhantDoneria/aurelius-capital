...
def test_dedup_identical_trial(ledger):
    t = Trial(family="value", hypothesis_id="h1", variant="ep", dataset_id="d1",
              period="2000-2010", params={"lookback": 12})
...
def test_cross_hypothesis_snooping_is_counted(ledger):
    ...
    for i in range(50):
        ledger.record(Trial(family="momentum", hypothesis_id=f"h{i}",
                            variant=f"v{i}"))
    assert ledger.effective_trials("momentum") == 50


def test_breakdown_axis_counts(ledger):
    ledger.record(Trial(family="q", hypothesis_id="h1", variant="a",
                        dataset_id="us", period="p1", params={"x": 1}))
    ledger.record(Trial(family="q", hypothesis_id="h2", variant="b",
                        dataset_id="eu", period="p2", params={"x": 2}))
    b = ledger.breakdown("q")
    assert b == {"hypotheses": 2, "variants": 2, "datasets": 2,
                 "periods": 2, "parameter_sets": 2, "trials": 2}
