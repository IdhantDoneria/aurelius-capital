...
def _fact(concept, unit, end, val, accn, filed, form="10-K", fp="FY", start=None):
    return {
        "cik": CIK, "security_id": None, "taxonomy": "us-gaap", "concept": concept,
        "unit": unit, "period_start": start, "period_end": end, "fiscal_year": end.year,
        "fiscal_period": fp, "value": float(val), "form": form, "accession": accn,
        "filing_date": filed, "frame": None,
    }
...
def test_parse_company_facts_preserves_restatements() -> None:
    doc = {
        ...
        "facts": {"us-gaap": {"Assets": {"units": {"USD": [
            {"end": "2019-09-28", "val": 338516000000, "accn": "a1", "fy": 2019, "fp": "FY", "form": "10-K", "filed": "2019-10-31"},
            {"end": "2019-09-28", "val": 338000000000, "accn": "a2", "fy": 2019, "fp": "FY", "form": "10-K/A", "filed": "2020-02-01"},
        ]}}}},
    }
...
def test_delayed_filing_not_visible_before_filed(store: FundamentalsStore) -> None:
    ...
    store.write_facts([_fact("StockholdersEquity", "USD", date(2019, 9, 28), 90488e6, "a1", date(2019, 10, 31))])
...
def test_restatement_knowledge_date(store: FundamentalsStore) -> None:
    store.write_facts([
        ...
        _fact("Assets", "USD", date(2019, 9, 28), 338000e6, "a2", date(2020, 2, 1), form="10-K/A"),
    ])
    eng = FundamentalsEngine(store)
    ...
    assert eng.fundamental_as_of(CIK, "assets", date(2020, 3, 1), knowledge_date=date(2019, 12, 1)) == 338516e6
...
def test_duplicate_filing_detected(store: FundamentalsStore) -> None:
    store.write_facts([
        _fact("Assets", "USD", date(2019, 9, 28), 338516e6, "a1", date(2019, 10, 31)),
        _fact("Assets", "USD", date(2019, 9, 28), 338516e6, "a1b", date(2019, 10, 31)),  # same value, 2 accns
    ])
    rep = check(store, CIK)
...
def test_negative_shares_and_missing_required(store: FundamentalsStore) -> None:
    store.write_facts([_fact("CommonStockSharesOutstanding", "shares", date(2019, 9, 28), -100, "a1", date(2019, 10, 31))])
    rep = check(store, CIK)
...
def test_cross_section_as_of_matches_per_cik_and_is_pit(store: FundamentalsStore) -> None:
    store.write_facts([
        ...
        {**_fact("Assets", "USD", date(2019, 9, 28), 200.0, "b1", date(2019, 10, 31)), "cik": "999"},
        # a future filing that must not leak into a 2019 cross-section:
        {**_fact("Assets", "USD", date(2020, 9, 28), 300.0, "b2", date(2020, 10, 31)), "cik": "999"},
    ])
...
def _bar(ticker, day, close):
    c = Decimal(str(close))
    return {"symbol": ticker, "timestamp": datetime(2019, 11, day, tzinfo=timezone.utc),
            "frequency": "1d", "open": c, "high": c, "low": c, "close": c, "volume": Decimal("1000")}
...
def test_pit_market_cap_and_enterprise_value(store: FundamentalsStore) -> None:
    ...
    try:
        store.write_facts([
            _fact("CommonStockSharesOutstanding", "shares", date(2019, 9, 28), 4_600_000_000, "a1", date(2019, 10, 31)),
            _fact("LongTermDebt", "USD", date(2019, 9, 28), 92_000e6, "a1", date(2019, 10, 31)),
            _fact("CashAndCashEquivalentsAtCarryingValue", "USD", date(2019, 9, 28), 48_000e6, "a1", date(2019, 10, 31)),
        ])
...
def test_market_cap_resolves_pit_ticker_from_security_master(store: FundamentalsStore) -> None:
    ...
    try:
        ...
        sid = make_security_id(isin="US0000000001", figi=None, ticker="OLD", exchange="XNAS", first_date=date(2019, 1, 1))
        sm.register(Security(security_id=sid, ticker="OLD", exchange="XNAS", isin="US0000000001"),
                    valid_from=date(2019, 1, 1))
        store.write_facts([_fact("CommonStockSharesOutstanding", "shares", date(2019, 9, 28), 1_000_000, "a1", date(2019, 10, 31))])
...
def test_factor_inputs_are_pit(store: FundamentalsStore) -> None:
    ...
    try:
        store.write_facts([
            _fact("StockholdersEquity", "USD", date(2019, 9, 28), 90_000e6, "a1", date(2019, 10, 31)),
            _fact("Assets", "USD", date(2019, 9, 28), 338_000e6, "a1", date(2019, 10, 31)),
            _fact("NetIncomeLoss", "USD", date(2019, 9, 28), 55_000e6, "a1", date(2019, 10, 31)),
        ...
        assert empty["roe"] is None and empty["market_cap"] is None
    finally:
        prices.close()
