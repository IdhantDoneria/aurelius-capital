...
def _form4_doc(name, shares, price, code, ad):
    return {
        ...
        "reportingOwner": {
            "reportingOwnerId": {"rptOwnerName": name},
...
def rig():
    ...
    sm.register(
        Security(security_id=sid, ticker="AAPL", exchange="XNAS", isin="US0378331005"),
    ...
    eng = ResearchMatrixEngine(
        universe=uni, fundamentals=fund, insiders=ins, prices=prices, cik_map={sid: CIK}
    )
    yield {
        "eng": eng,
        "sm": sm,
...
def test_future_fundamental_not_visible(rig):
    ...
    assert before.frame.loc[sid, "book_value"] is None or math.isnan(
        _num(before.frame.loc[sid, "book_value"])
    )
...
def test_future_insider_not_visible(rig):
    ...
    rows = parse_form4(
        _form4_doc("COOK", 1000, 150.0, "P", "A"),
        accession="i1",
...
def test_delisted_included_historically_excluded_now(rig):
    sm = rig["sm"]
    dead = make_security_id(
        isin="US_DEAD", figi=None, ticker="DEAD", exchange="XNYS", first_date=date(2005, 1, 1)
    )
    sm.register(
        Security(security_id=dead, ticker="DEAD", exchange="XNYS"), valid_from=date(2005, 1, 1)
    )
...
def test_identity_migration(rig):
    ...
    sm.add_identity_change(
        sid, new_ticker="APPL2", exchange="XNAS", valid_from=date(2021, 1, 1), reason="rename"
    )
    rig["prices"].write_raw_bars(
        [
            {
                "symbol": "APPL2",
                "timestamp": datetime(2021, 6, 30),
                "frequency": "1d",
                "open": 100,
                "high": 101,
                "low": 99,
...
def test_missing_data_isolated(rig):
    ...
    empty = make_security_id(
        isin="US_EMPTY", figi=None, ticker="EMPT", exchange="XNYS", first_date=date(2000, 1, 1)
    )
    sm.register(
        Security(security_id=empty, ticker="EMPT", exchange="XNYS"), valid_from=date(2000, 1, 1)
    )
