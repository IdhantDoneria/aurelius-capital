...
def _txn(**over):
    doc = _form4_doc(
        over.pop("name", "COOK TIMOTHY"),
        over.pop("shares", 1000),
        over.pop("price", 150.0),
        ad=over.pop("ad", "A"),
        owned=over.pop("owned", 5000),
        code=over.pop("code", "P"),
    )
    rows = parse_form4(
        doc,
        accession=over.pop("accession", "a1"),
        filing_date=over.pop("filing_date", date(2026, 1, 3)),
        acceptance_datetime=over.pop("acceptance", datetime(2026, 1, 3, 18, 0, 0)),
        security_id=SID,
        form_type=over.pop("form_type", "4"),
    )
    return rows
...
def test_filing_delay_isolation(store: InsiderStore) -> None:
    ...
    assert visible[0]["transaction_code"] == "P"
...
def test_amendment_returns_latest(store: InsiderStore) -> None:
    ...
    store.write_transactions(
        _txn(accession="a1", shares=100, owned=100, acceptance=datetime(2026, 1, 3, 18, 0, 0))
    )
    store.write_transactions(
        _txn(
            accession="a2",
...
def test_cluster_buying_aggregation(store: InsiderStore) -> None:
    for i, name in enumerate(["COOK TIMOTHY", "MAESTRI LUCA", "OBRIEN DEIRDRE"]):
        store.write_transactions(
            _txn(name=name, accession=f"a{i}", shares=1000 * (i + 1), price=150.0)
        )
    # one sale by a fourth insider
    store.write_transactions(
        _txn(name="WILLIAMS JEFF", accession="s1", shares=500, price=150.0, ad="D", code="S")
    )
    eng = InsiderEngine(store, cluster_threshold=3)
    sig = eng.insider_signal_as_of(SID, date(2026, 1, 4))
    assert sig.purchases == 3
    assert sig.sales == 1
